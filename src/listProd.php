  ...
  if($_SESSION['admin'] == "admin") redirect("admin-home.php");
		...
		if(isset($_GET['category']) && !($_GET['category']==="[]")){
			...
			if(!is_array($categories)){
				echo("<h3>The categories were not sent correctly. <a href=\"shop.php\">View new listings</a></h3>");
				mysqli_close($con);
				die();
			}
			foreach($categories as $c){ //validate categories
				if(!in_array($c,$allowedCategories)){
					echo("<h3>An invalid category '$c' was found in the filter. <a href=\"shop.php\">View new listings</a></h3>");
					mysqli_close($con);
					die();
				}
			}
